...
export default function TradingJournal() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        try {
          const k = await storage.get('config:finnhub_key');
    ...
    if (view === 'recaps') {
      const loadRecaps = async () => {
        try {
          ...
          const weekly = await storage.get(weekKey);
          const monthly = await storage.get(monthKey);
  ...
  const parseTlg = (text) => {
    ...
    const trades = [];
    const diag = { totalLines: 0, candidateLines: 0, sample: null };
    ...
    for (const rawLine of rawLines) {
      ...
      if (!upper.startsWith('STK_TRD|') && !upper.startsWith('OPT_TRD|')) continue;
      diag.candidateLines++;
      const f = line.split('|');
      if (f.length < 15) {
        if (!diag.sample) diag.sample = { reason: `expected at least 15 pipe-delimited fields, found ${f.length}`, line: line.slice(0, 160) };
        continue;
      }
      const dateRaw = (f[7] || '').trim();
      let dateKey = null;
      if (/^\d{8}$/.test(dateRaw)) {
      ...
      if (!dateKey) {
        if (!diag.sample) diag.sample = { reason: `unrecognized date value "${dateRaw}" (expected YYYYMMDD)`, line: line.slice(0, 160) };
        continue;
      }
      const time = (f[8] || '').trim().slice(0, 8); // trim any fractional seconds
      trades.push({
        id: f[1],
        symbol: f[2],
        description: f[3],
  ...
  const handleTlgImport = async (e) => {
    ...
    try {
      ...
      if (allParsed.length === 0) {
        const totalLines = diags.reduce((s, d) => s + d.totalLines, 0);
        const candidateLines = diags.reduce((s, d) => s + d.candidateLines, 0);
        const sample = diags.map(d => d.sample).find(Boolean);
        let msg;
        if (candidateLines === 0) {
          msg = `No STK_TRD/OPT_TRD lines found in ${totalLines} line${totalLines === 1 ? '' : 's'}. Is this an Interactive Brokers .tlg (Trade Log) export?`;
        } else if (sample) {
          msg = `Found ${candidateLines} trade line${candidateLines === 1 ? '' : 's'} but couldn't parse ${candidateLines === 1 ? 'it' : 'any of them'} — ${sample.reason}. Line: "${sample.line}"`;
        } else {
          msg = 'No trades found. Is this an Interactive Brokers .tlg file?';
        }
  ...
  // Export every stored record (entries, trades, recaps, config) as one JSON file.
  // Use this to move data between devices/browsers, or to back it up.
  const handleExportData = async () => {
    setSyncStatus({ type: 'loading', msg: 'Exporting…' });
    try {
      ...
      const payload = { app: 'trading-journal', version: 1, exportedAt: new Date().toISOString(), data };
      const blob = new Blob([JSON.stringify(payload, null, 2)], { type: 'application/json' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `trading-journal-backup-${todayKey()}.json`;
      document.body.appendChild(a);
      a.click();
      a.remove();
      URL.revokeObjectURL(url);
      setSyncStatus({ type: 'success', msg: `Exported ${keys.length} record${keys.length === 1 ? '' : 's'}.` });
      setTimeout(() => setSyncStatus(null), 5000);
    } catch (err) {
      setSyncStatus({ type: 'error', msg: 'Export failed: ' + err.message });
      setTimeout(() => setSyncStatus(null), 5000);
    }
  };
  ...
  const handleImportDataFile = async (e) => {
    ...
    e.target.value = '';
    if (!file) return;
    setSyncStatus({ type: 'loading', msg: 'Importing backup…' });
    try {
      const text = await file.text();
      const payload = JSON.parse(text);
      const data = payload && payload.data && typeof payload.data === 'object' ? payload.data : null;
      if (!data) throw new Error("This doesn't look like a Trading Journal export file.");
      const keys = Object.keys(data);
      if (keys.length === 0) throw new Error('The file has no records to import.');
      for (const key of keys) {
        await storage.set(key, data[key]);
      }
      setSyncStatus({ type: 'success', msg: `Imported ${keys.length} record${keys.length === 1 ? '' : 's'}. Reloading…` });
  ...
  const saveEntry = async () => {
    ...
    try {
      await storage.set(`entry:${currentDate}`, JSON.stringify(toSave));
  ...
  const deleteEntry = async (dateKey) => {
    ...
    try {
      await storage.delete(`entry:${dateKey}`);
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'radial-gradient(ellipse at top, #0f1e3d 0%, #060a14 60%)', color: '#e4e4e7', fontFamily: '"Inter", -apple-system, sans-serif' }}>
      <Head>
        <title>Trading Journal</title>
        <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, viewport-fit=cover" />
        <meta name="description" content="Personal trading journal with morning check-ins, analytics, and trade tracking." />
      </Head>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        @media (max-width: 860px) {
          ...
          .sidebar {
            ...
            z-index: 50;
            /* Hint that there are more tabs than fit on screen — the row
               scrolls horizontally (swipe), it isn't the full tab list. */
          ...
          .sidebar-brand, .sidebar-extra, .sidebar-footer { display: none; }
          .sidebar .side-btn {
            width: auto;
            flex-shrink: 0;
            padding: 8px 12px;
          ...
          main {
            padding: 20px 16px calc(96px + env(safe-area-inset-bottom)) !important;
            max-width: 100% !important;
          }
          header h1 { font-size: 32px !important; }
          /* Fixed two-column layouts don't fit a phone width — collapse to
             one column instead of overflowing (which was forcing mobile
             browsers to zoom the whole page out to fit the wide content). */
          .journal-grid { grid-template-columns: 1fr !important; }
          /* The calendar grid's fixed 80px cell floor forces 7 columns
             wider than a phone screen — let cells shrink (aspect-ratio
             keeps them square) so the whole month still fits. */
          .calendar-cell { min-height: 32px !important; padding: 4px !important; }
          /* The sticky Save bar can visually collide with the fixed bottom
             tab bar on small screens (and intercept taps meant for it).
             Let it sit in normal flow instead — main's bottom padding
             above already leaves room to clear the tab bar. */
          .save-bar {
      ...
      <div style={{ position: 'relative', display: 'flex', minHeight: '100vh' }}>
        ...
        <aside className="sidebar">
          <div className="sidebar-brand" style={{ padding: '0 12px 28px' }}>
          ...
          <div className="sidebar-extra" style={{ marginTop: 16, padding: '0 4px' }}>
          ...
          <div className="sidebar-extra" style={{ marginTop: 10, padding: '0 4px', display: 'flex', gap: 8 }}>
            <button onClick={handleExportData} className="nav-btn" style={{ flex: 1, justifyContent: 'center', fontSize: 12, padding: '8px 10px' }}>
              <ArrowDownToLine size={13} style={{ transform: 'rotate(180deg)' }} /> Export
            </button>
            <label className="nav-btn" style={{ flex: 1, justifyContent: 'center', fontSize: 12, padding: '8px 10px', cursor: 'pointer' }}>
              <Upload size={13} /> Import
              <input type="file" accept=".json" onChange={handleImportDataFile} style={{ display: 'none' }} />
            </label>
          </div>
          {syncStatus && (
            <div className="sidebar-extra" style={{
              margin: '8px 4px 0', padding: '8px 12px', borderRadius: 6, fontSize: 11, lineHeight: 1.4,
              background: syncStatus.type === 'error' ? 'rgba(239,68,68,0.1)' : syncStatus.type === 'success' ? 'rgba(16,185,129,0.1)' : 'rgba(255,255,255,0.04)',
              color: syncStatus.type === 'error' ? '#fca5a5' : syncStatus.type === 'success' ? '#6ee7b7' : '#a1a1aa',
              border: `1px solid ${syncStatus.type === 'error' ? 'rgba(239,68,68,0.2)' : syncStatus.type === 'success' ? 'rgba(16,185,129,0.2)' : 'rgba(255,255,255,0.06)'}`,
            }}>
              {syncStatus.msg}
            </div>
          )}
        ...
        {/* JOURNAL VIEW */}
        {view === 'journal' && (
          <div className="fade-in journal-grid" style={{ display: 'grid', gridTemplateColumns: 'minmax(280px, 1fr) 2fr', gap: 20, alignItems: 'flex-start' }}>
          ...
          return (
            <div className="fade-in" style={{ display: 'grid', gap: 20 }}>
              ...
              <div className="card">
                ...
                <button
                  onClick={() => {
                    storage.set(weekKey, weeklyRecap);
                  }}
              ...
              <div className="card">
                ...
                <button
                  onClick={() => {
                    storage.set(monthKey, monthlyRecap);
                  }}
                  style={{ padding: '10px 20px', background: '#3b82f6', color: '#fff', border: 'none', borderRadius: 6, cursor: 'pointer', fontWeight: 600, fontSize: 13 }}
                >
                  Save Monthly Entry
        ...
        {(view === 'checkin' || view === 'journal') && (
          <div className="save-bar" style={{ position: 'sticky', bottom: 20, marginTop: 32, display: 'flex', justifyContent: 'flex-end', gap: 12, alignItems: 'center' }}>
            {savedFlash && (
